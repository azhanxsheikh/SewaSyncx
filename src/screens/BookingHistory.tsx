...
export default function BookingHistory({ navigate }: Props) {
  ...
  const handleTrackLive = (b: BookingRecord) => {
    ...
    const validStatus = ['accepted', 'en-route', 'arrived', 'in-progress'].includes(raw)
    ...
    replaceJob({
      ...
      priority: (b.priority as 'low' | 'medium' | 'high' | 'critical') || 'medium',
      status: validStatus,
      ...
      <div className="max-w-md mx-auto px-4 pt-4 space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((b) => {
            ...
            const isActive =
              ['accepted', 'en-route', 'arrived', 'in-progress'].includes(raw) ||
              ['Accepted', 'En Route', 'Arrived', 'In Progress'].includes(b.status);
