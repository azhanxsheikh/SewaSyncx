  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 px-4 pt-5 pb-28 max-w-md mx-auto w-full">
        ...
        {/* Payment methods */}
        <p className="font-display font-700 text-gray-900 mb-3">Select payment method</p>
          ...
          {methods.map((m) => (
            <button
              ...
              className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 text-left transition-all ${
                selected === m.id ? 'border-blue-400 bg-blue-50' : 'border-gray-100 bg-white hover:border-gray-200'
              }`}
            >
              ...
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                ...
                {selected === m.id && <div className="w-2.5 h-2.5 rounded-full bg-blue-500" />}
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="bg-gray-50 rounded-xl p-3 flex gap-2 items-start text-xs text-gray-500">
          ...
          <span>Payments are 256-bit encrypted and processed through secure RBI-approved gateways.</span>
        </div>
      </div>
