...
export default function Chat({ navigate, onBack }: Props) {
  ...
  const sendMessage = (text: string) => {
    if (!text.trim()) return;
    const newMsg = { id: `m${Date.now()}`, sender: 'user', text, time: new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) };
    setMessages(prev => [...prev, newMsg]);
    setInput('');

    // Simulate tech reply
    setTimeout(() => {
      const reply = { id: `r${Date.now()}`, sender: 'tech', text: "Got it! I'll be there soon.", time: new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) };
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-100">
        <div className="max-w-md mx-auto flex items-center px-4 py-3 gap-3">
          ...
          <img src={tech.photo} alt={tech.name} className="w-9 h-9 rounded-full object-cover" />
          <div className="flex-1">
            <p className="font-display font-700 text-gray-900 text-sm">{tech.name}</p>
            <p className="text-xs text-emerald-500 font-500">● Online · Electrician</p>
          </div>
          ...
          <button onClick={() => navigate('sos-tracking')} className="p-2 rounded-xl bg-blue-50 hover:bg-blue-100 transition-colors">
        ...
        <div className="max-w-md mx-auto px-4 pb-2.5">
          <div className="bg-blue-50 rounded-xl px-3 py-2 flex items-center justify-between">
            <p className="text-xs text-blue-700 font-500">🛵 Rahul is 5 minutes away</p>
            <button onClick={() => navigate('sos-tracking')} className="text-blue-600 text-xs font-600 hover:underline">Track</button>
          </div>
        </div>
      </div>
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} gap-2`}>
              {!isUser && (
                <img src={tech.photo} alt="" className="w-7 h-7 rounded-full object-cover flex-shrink-0 self-end" />
              )}
              <div className={`max-w-[75%]`}>
                ...
                <p className={`text-[10px] text-gray-400 mt-1 ${isUser ? 'text-right' : 'text-left'}`}>{msg.time}</p>
              </div>
            </div>
          );
        })}
