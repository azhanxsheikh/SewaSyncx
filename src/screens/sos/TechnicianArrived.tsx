...
export default function TechnicianArrived({ navigate }: Props) {
  ...
  const techRating = job?.technicianRating ?? techProfile?.rating ?? 4.9
  const techJobs = job?.technicianTotalJobs ?? techProfile?.jobs ?? 64
  const serviceName =
    ...
    (job?.service
      ...
      : "AC Repair")
  const vehicleText =
    job?.technicianVehicle ||
    techProfile?.vehicle ||
    "Two-Wheeler / Scooter · UP 16 AB 1234"
  const jobId = job?.id
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6">
      ...
      <p className="text-gray-500 text-sm text-center mb-8">
        {techName} is at your door. Please verify his identity before starting.
      </p>
      ...
      <div className="w-full max-w-xs bg-white border-2 border-emerald-200 rounded-2xl p-5 shadow-md shadow-emerald-50 mb-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          />
          <div>
            <h3 className="font-display font-700 text-gray-900">{techName}</h3>
            <div className="flex items-center gap-1 text-xs text-gray-500 mt-0.5">
              <span className="text-amber-400">★</span>
              <span>{techRating}</span>
              <span>·</span>
              <span>{techJobs.toLocaleString("en-IN")} jobs</span>
            </div>
        ...
        <div className="space-y-2 text-sm border-t border-gray-100 pt-4">
          <div className="flex justify-between">
            ...
            <span className="font-600 text-gray-900">{serviceName}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Vehicle</span>
            <span className="font-600 text-gray-900 text-xs">
              {vehicleText}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Job ID</span>
            <span className="font-600 text-gray-900">{jobId}</span>
          </div>
        </div>

        <div className="mt-4 bg-blue-50 rounded-xl p-3">
          <p className="text-xs text-blue-700 font-500">
            💡 Tip: Match the profile photo above with the person at your door
