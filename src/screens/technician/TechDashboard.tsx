...
export default function TechDashboard({ onOpenAlerts, onOpenActive }: Props) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-3">
      {[['Today\'s jobs', active ? '1' : '0'], ['Earnings', jobHistory.length ? '₹648' : '₹0'], ['Rating', '4.9 ★']].map(([label, value]) => <div key={label} className="rounded-2xl border border-slate-800 bg-slate-900 p-5"><p className="text-sm text-slate-400">{label}</p><p className="mt-2 font-display text-2xl font-800">{value}</p></div>)}
    </div>
