...
export function BookingDetailsModal({
  ...
  const isCancellable =
    ...
    !booking.status.toLowerCase().includes("arrived")
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
        <div className="w-full max-w-md bg-white rounded-2xl border border-gray-100 shadow-2xl p-6 space-y-5 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`px-3 py-1 rounded-full text-xs font-700 border ${statusColorClass}`}
            >
              {booking.status}
            </span>
            <span
              className={`px-3 py-1 rounded-full text-xs font-700 border uppercase tracking-wider ${badgeClass}`}
            >
          ...
          <div className="bg-gray-50 rounded-xl p-3.5 flex items-center justify-between border border-gray-100">
            <div>
              ...
              <p className="font-600 text-sm text-gray-900 mt-0.5">
                {booking.technician || "Pending Assignment"}
              </p>
              {booking.technicianPhone && (
                <p className="text-xs text-blue-600 font-500 mt-0.5">
                  {booking.technicianPhone}
                </p>
              )}
            </div>
            <div className="text-right">
              ...
              <p className="font-display font-800 text-base text-gray-900 mt-0.5">
                {booking.amount}
              </p>
            </div>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {booking.addressNotes && (
                    <div className="mt-2 rounded-lg bg-white p-2.5 border border-gray-200/70 text-xs text-gray-700 leading-relaxed shadow-2xs">
                      <span className="font-700 text-amber-800">
                        Landmark / Access:{" "}
                      </span>
          ...
          <div className="space-y-1.5">
            ...
            <div className="p-3.5 rounded-xl bg-gray-50 border border-gray-100 text-sm text-gray-700 leading-relaxed">
              {booking.description ||
                "Routine service inspection and repair requested at client address."}
            </div>
          </div>
          ...
          {isCancellable && (
            <div className="pt-1">
              <button
                ...
                className="w-full py-2.5 rounded-xl border border-red-300 bg-white hover:bg-red-50 text-red-600 font-display font-600 text-xs transition-colors flex items-center justify-center gap-1.5"
              >
                <span>✕</span>
