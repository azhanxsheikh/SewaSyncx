...
export default function OperationsMetricsView() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* SECTION 1: Fare Variance Tracker */}
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          {mockFareVarianceSummaries.map((summary) => (
            <div
      ...
      {/* SECTION 2: Pairwise Collusion Anomaly Cards */}
      ...
      {/* SECTION 3: Photo Mismatch Reviewer */}
      <div className="space-y-4 pt-4 border-t border-slate-800">
        <div>
          <h3 className="font-display text-lg font-800 text-white">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <p className="text-[10px] text-slate-400 uppercase">Pre-Work Upload</p>
                  <img
                ...
                <div className="space-y-1">
                  <p className="text-[10px] text-slate-400 uppercase">Post-Work Upload</p>
                  <img
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 space-y-1 text-xs">
                <div className="flex justify-between">
                  <span className="text-slate-500">EXIF Location Delta:</span>
                  <span
                    className={`font-700 ${
                      review.exifGpsDeltaMeters > 150 ? 'text-red-500' : 'text-emerald-300'
                    }`}
                  >
                    {review.exifGpsDeltaMeters} meters {review.exifGpsDeltaMeters > 150 ? '(FLAGGED > 150m)' : '(Valid)'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Perceptual Hash Dist:</span>
                  <span
                    className={`font-700 ${
                      review.phashDistance === 0 ? 'text-red-500' : 'text-slate-300'
                    }`}
                  >
                    {review.phashDistance} {review.phashDistance === 0 ? '(DUPLICATE PHOTO FLAG)' : ''}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Timestamp Alignment:</span>
                  <span
                    className={`font-700 ${
                      review.timestampAnomaly ? 'text-amber-400' : 'text-emerald-300'
                    }`}
                  >
                    {review.timestampAnomaly ? 'Outside shift window' : 'Verified in window'}
                  </span>
                </div>
              </div>
              ...
              {review.status === 'pending_review' && (
                <div className="flex gap-2 pt-2 border-t border-slate-800">
                  <button
                    ...
                    className="flex-1 rounded-xl bg-emerald-500 py-2 text-xs font-700 text-white hover:bg-emerald-600 transition-colors"
                  >
                    Approve Evidence
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-xl bg-red-500 py-2 text-xs font-700 text-white hover:bg-red-600 transition-colors"
                  >
                    Escalate to Dispute
                  </button>
                </div>
