...
import {
  DispatchProvider,
  useDispatch,
} from "../../../src/context/DispatchContext"
...
import CostAdditionApprovalModal from "./components/settlement/CostAdditionApprovalModal"
...
function ClientAppInner() {
  ...
  const { activeRequest, status: activeStatus } = useActiveRequest()
  ...
  const [familySubScreen, setFamilySubScreen] =
    useState<FamilySubScreen>("list")
  const [selectedFamilyMember, setSelectedFamilyMember] = useState("f1")
  const [scheduledSubScreen, setScheduledSubScreen] =
    useState<ScheduledSubScreen>("category")

  useEffect(() => {
    const currentStatus = activeStatus || job?.status
    if (!currentStatus) return

    if (currentStatus === "accepted") {
      if (screen === "sos-finding" || screen === "sos-confirmation") {
        navigate("sos-assigned")
      }
    } else if (currentStatus === "en-route") {
      if (
        screen === "sos-finding" ||
        screen === "sos-confirmation" ||
        screen === "sos-assigned"
      ) {
      ...
      if (
        screen === "sos-inprogress" ||
        screen === "sos-additional-cost" ||
        screen === "sos-arrived" ||
        screen === "sos-tracking"
      ) {
  ...
  }, [activeStatus, job?.status, screen])

  useEffect(() => {
    if (status === "signed-in" && role && role !== "client") {
      void signOut()
      setDeniedMsg(
        "ACCESS DENIED: Client portal is restricted to registered client accounts.",
      )
    }
  }, [status, role, signOut])
  ...
  const screenMap: Record<Screen, ReactElement> = {
    ...
    "sos-chat": (
      <Chat navigate={navigate} onBack={() => navigate("sos-tracking")} />
    ),
    ...
    "sos-invoice": (
      <DigitalInvoice
        ...
        onBack={() => {
          clearJustCompleted()
          navigate(
            prevScreen === "sos-completed" ? "bookings" : prevScreen || "home",
          )
        }}
      />
    ),
    "sos-payment": (
      <Payment navigate={navigate} onBack={() => navigate("sos-invoice")} />
    ),
    ...
    notifications: (
      <Notifications navigate={navigate} onBack={() => navigate(prevScreen)} />
    ),
    admin: (
      <div className="p-4 text-center text-sm text-gray-500">
        Redirecting to Admin Ops Console...
      </div>
    ),
  }
    ...
    <SOSRouteErrorBoundary>
      <div className="bg-gray-100 min-h-screen">
        <div className="max-w-md mx-auto bg-white min-h-screen relative shadow-2xl shadow-gray-400/20">
          ...
          <CostAdditionApprovalModal
            activeRequestId={activeRequest?.id || job?.id}
            job={job}
          />
